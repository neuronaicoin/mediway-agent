...
def _find_font(bold=True):
    """DejaVu fontunu sistemde otomatik bul (Railway/Linux/Mac farklı yerlerde tutar)."""
    name = "DejaVuSans-Bold.ttf" if bold else "DejaVuSans.ttf"
    candidates = [
        f"/usr/share/fonts/truetype/dejavu/{name}",
        f"/usr/share/fonts/dejavu/{name}",
        f"/nix/store/*/share/fonts/truetype/{name}",
    ...
    for pat in candidates:
        ...
        if hits:
            return hits[0]
    # Son çare: sistemde herhangi bir DejaVu bold/regular ara
    fallback = glob.glob(f"/usr/share/fonts/**/{name}", recursive=True) + \
               glob.glob(f"/nix/**/{name}", recursive=True)
    if fallback:
        return fallback[0]
    # Hiçbiri yoksa Pillow'un default'una düş (Türkçe karakter zayıf olur ama çöker yerine çalışır)
    return None
