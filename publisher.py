"""
================================================================
 MEDIWAY AGENT — YAYINLAMA MODÜLÜ (TAM)
 (publisher.py)
================================================================

AKIŞ: Görsel/videoyu sitenin GİZLİ klasörüne FTP ile yükle →
URL'i Meta'ya ver → yayınla → dosyayı sunucudan sil.
...
ENVIRONMENT VARIABLE'LAR (hiçbiri koda yazılmaz):
  ...
  FB_PAGE_TOKEN   → Facebook sayfa yayını için (IG token'dan AYRI olabilir)
  FTP_HOST / FTP_USER / FTP_PASS / FTP_DIR
  SITE_BASE_URL   → görsellerin web adresi (örn. https://mediwayturkey.com/ig-temp)
----------------------------------------------------------------
...
FTP_DIR  = os.environ.get("FTP_DIR", "ig-temp")
...
def upload_to_site(local_path):
    """Dosyayı gizli klasöre yükler, web URL'ini ve dosya adını döner."""
...
def delete_from_site(filename):
    """Yayınlanan dosyayı sunucudan siler."""
    try:
        ftp = ftplib.FTP(FTP_HOST)
        ftp.login(FTP_USER, FTP_PASS)
        ftp.cwd(FTP_DIR)
...
def _check_settings(extra=None):
    ...
    if not SITE_BASE_URL: missing.append("SITE_BASE_URL")
    for e in (extra or []):
